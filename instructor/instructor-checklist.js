...
const checklistFields = [
  ...
  { key: "finalStepCompleted",    label: "Final Step: In-person walkthrough & driving portion completed" }
];

// === MAIN CHECKLIST REVIEW MODAL ===
export async function renderChecklistReviewForInstructor(studentEmail, container = document.body, role = "instructor") {
  ...
  let studentData = {}, eldtData = {}, auditTrail = [];
  try {
    // Profile
    ...
    if (progressSnap.exists()) {
      ...
      auditTrail = histSnap.docs.map(doc => doc.data()).sort((a, b) =>
        (b.updatedAt?.toDate?.() || new Date(b.updatedAt)) - (a.updatedAt?.toDate?.() || new Date(a.updatedAt))
      );
    }
  } catch (e) {
          ...
          <div style="max-height:120px;overflow:auto;font-size:0.97em;padding-top:8px;">
            ${auditTrail.map(entry => `
              <div>
                ...
                ${Object.entries(entry).filter(([k,v]) => k !== "updatedAt" && k !== "role" && k !== "updatedBy")
                  .map(([k,v]) => `<span style="margin-left:7px;">${k}: <b>${v && typeof v==="boolean" ? (v ? "✔" : "--") : v}</b></span>`).join("")}
              </div>
            `).join("")}
          </div>
        </details>
  ...
  modal.querySelector("#close-checklist-modal")?.addEventListener("click", closeModal);
  modal.focus();
  ...
  modal.querySelector("#checklist-review-form").onsubmit = async e => {
    ...
    let updateObj = {};
    checklistFields.forEach(f => { updateObj[f.key] = !!fd.get(f.key); });
    ...
    try {
      ...
      showToast("Failed to save checklist: " + (err.message || err), 4400, "error");
    }
  };
