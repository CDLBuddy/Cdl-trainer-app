...
async function fetchLogs({
  ...
  if (school)
    logs = logs.filter((l) => l.schoolId === school || l.schoolName === school);
  if (keyword)
    logs = logs.filter(
      (l) =>
...
function exportLogsToCSV(logs) {
  ...
  link.href = url;
  link.download = `cdl-logs-${Date.now()}.csv`;
...
export async function renderLogs(container = document.getElementById('app')) {
  ...
  const allRoles = [
    ...new Set(logs.map((l) => l.userRole).filter(Boolean)),
  ].sort();
  const allSchools = [
    ...new Set(logs.map((l) => l.schoolName || l.schoolId).filter(Boolean)),
  ].sort();
