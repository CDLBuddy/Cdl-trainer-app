...
export async function renderBilling(container = document.getElementById("app")) {
      ...
      <div class="dashboard-card">
        ...
        <table class="schools-billing-table" style="width:100%;margin-top:1.4rem;">
          ...
          <tbody>
            ...
            ` : schools.map(school => {
              // --- Overdue badges ---
              const overdue = school.renewalDate && new Date(school.renewalDate) < new Date();
              return `
                <tr>
                  <td>${school.name || "-"}</td>
                  ...
                  <td>
                    ${school.licensingDocs && school.licensingDocs.length
                      ? school.licensingDocs.map((url, i) => `<a href="${url}" target="_blank">Lic ${i+1}</a>`).join(", ")
  ...
  document.getElementById("export-billing-csv")?.addEventListener("click", () => {
    const rows = [
      ["School Name", "Location", "TPR ID", "Plan", "Status", "Renewal", "Last Payment", "Invoice", "Licensing Docs", "Notes"]
    ].concat(
      schools.map(school => [
        school.name || "",
        school.location || "",
        ...
        school.invoiceUrl || "",
        (school.licensingDocs && school.licensingDocs.length ? school.licensingDocs.join(", ") : ""),
        school.billingNotes || ""
      ])
    );
  ...
  container.querySelectorAll('.btn[data-action="view"]').forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      showBillingModal(schools.find(s => s.id === schoolId), "view", container);
    });
  });
  container.querySelectorAll('.btn[data-action="edit"]').forEach(btn => {
    btn.addEventListener("click", () => {
      const schoolId = btn.getAttribute("data-school-id");
      showBillingModal(schools.find(s => s.id === schoolId), "edit", container);
    });
  });
}
...
function showBillingModal(school, mode = "view", container) {
  const modalRoot = document.getElementById("billing-modal-root") || document.body;
  const isEdit = mode === "edit";
  const renewal = school.renewalDate ? new Date(school.renewalDate).toISOString().slice(0,10) : "";
  const payment = school.lastPaymentDate ? new Date(school.lastPaymentDate).toISOString().slice(0,10) : "";
      ...
      <form id="billing-form">
        ...
        <label>Licensing Docs: 
          ${isEdit
            ...
            : (school.licensingDocs && school.licensingDocs.length
                ? school.licensingDocs.map((url, i) => `<a href="${url}" target="_blank">License ${i+1}</a>`).join(", ")
                : `<span class="badge badge-neutral">None</span>`)}
        </label>
        ...
        <label>Manual Override: 
          ${isEdit
            ...
            : (school.manualOverride ? `<span>${school.manualOverride}</span>` : `<span class="badge badge-neutral">None</span>`)}
        </label>
  ...
  modal.querySelector(".modal-close").onclick =
    modal.querySelector("#close-billing-modal").onclick = () => modal.remove();
  ...
  if (isEdit) {
    modal.querySelector("#billing-form").onsubmit = async e => {
      ...
      const updated = {
        ...
        licensingDocs: fd.get("licensingDocs") ? fd.get("licensingDocs").split(',').map(x => x.trim()) : [],
