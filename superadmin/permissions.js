...
const PERMISSIONS_LIST = [
  "manage_users", "manage_schools", "edit_compliance",
  "view_billing", "manage_settings", "view_reports",
  "assign_roles", "edit_students", "read_only"
];
...
async function fetchUsers(search = "") {
  ...
  if (search) query = query.where("keywords", "array-contains", search.toLowerCase());
...
async function fetchAuditLog(userId) {
  try {
    const snap = await db.collection("users").doc(userId).collection("permissionsLog").orderBy("timestamp", "desc").get();
  ...
  modal.innerHTML = `
    <div class="modal-card" style="max-width:530px;">
      ...
      <div style="max-height:320px;overflow-y:auto;">
        ${log.length === 0 ? "<div>No changes found.</div>" : log.map(entry => `
          <div class="audit-row">
...
export async function renderPermissions(container = document.getElementById("app")) {
  if (!container) return;
                    ...
                    <tr data-user-id="${user.id}">
                      ...
                      <td>
                        <select class="role-select" data-user-id="${user.id}">
                          ${["superadmin","admin","instructor","student","custom"].map(role =>
  ...
  container.querySelectorAll('.save-role-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      const assignedSchools = Array.from(schoolSelect.selectedOptions).map(o => o.value);

      // Permissions (checkboxes)
      const permChecks = row.querySelectorAll('input[type="checkbox"][data-perm]');
      const permissions = Array.from(permChecks).filter(c => c.checked).map(c => c.dataset.perm);
      ...
      try {
        ...
        await db.collection("users").doc(userId).collection("permissionsLog").add({
        ...
        row.querySelector('td:nth-child(3) .role-badge').className = `role-badge ${newRole}`;
      } catch (e) {
  ...
  container.querySelectorAll('button[data-action="view-log"]').forEach(btn => {
    btn.addEventListener('click', async () => {
