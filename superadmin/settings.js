...
async function saveSettings(type, settings, schoolId = null) {
  if (type === "global") {
    ...
    await updateDoc(doc(db, "schools", schoolId), { settings }, { merge: true });
...
async function revokeApiKey(keyId) {
  await updateDoc(doc(db, "apiKeys", keyId), { active: false, revokedAt: serverTimestamp() });
  logSettingsChange("apiKey", { keyId, action: "revoked" });
}

// ======== SUPER ADMIN SETTINGS UI RENDERER ========
export async function renderSettings(container = document.getElementById("app")) {
  if (!container) return;
  setupNavigation();
      ...
      <details>
        ...
        <div>
          ...
          <ul class="api-key-list">
            ${apiKeys.length === 0 ? "<li>No API keys issued yet.</li>" :
              apiKeys.map(k => `
                <li>
  ...
  container.querySelector("#global-settings-form")?.addEventListener("submit", async e => {
    e.preventDefault();
  ...
  container.querySelector("#school-select")?.addEventListener("change", async (e) => {
    ...
    if (selectedSchoolId) {
      ...
      container.querySelector("#school-settings-form")?.addEventListener("submit", async evt => {
        evt.preventDefault();
      ...
      container.querySelector("#reset-school-settings")?.addEventListener("click", async () => {
  ...
  // Generate API Key
  container.querySelector("#api-key-form")?.addEventListener("submit", async e => {
    e.preventDefault();
    const fd = new FormData(e.target);
    const key = await createApiKey(fd.get("label"), fd.get("schoolId") || null);
  ...
  container.querySelector("#settings-log-list").innerHTML =
    logSnap.empty
      ? "(No changes yet.)"
      : logSnap.docs.map(l =>
