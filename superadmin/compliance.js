...
export async function renderComplianceCenter(container = document.getElementById("app")) {
  if (!container) return;
            ...
            ${
              schools.length === 0
                ...
                : schools.map(school => {
                  ...
                  const renewalBadge = daysToRenewal !== null
                    ? (daysToRenewal < 0
                    ...
                    : "-";
                  const daysSinceAudit = school.lastAuditDate ? daysUntil(school.lastAuditDate) * -1 : null;
                  const auditBadge = daysSinceAudit !== null
                    ? (daysSinceAudit > 365
                  ...
                  if (!school.curriculumFile) missing.push("Missing curriculum");
                  if (!school.instructorFiles || !school.instructorFiles.length) missing.push("No instructor files");
                  if (!school.humanTraffickingPolicy) missing.push("Missing trafficking cert");
                  if (!school.tprStatus || school.tprStatus !== "Active") missing.push("TPR problem");
                  if (daysToRenewal !== null && daysToRenewal < 0) missing.push("Renewal overdue");

                  // Health score (green/yellow/red)
                  let score = "green";
                  if (missing.length) score = missing.length > 2 ? "red" : "yellow";
  ...
  container.querySelectorAll('.btn[data-action="review"]').forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      showComplianceModal(schools.find(s => s.id === schoolId), "view", container);
    });
  });
  container.querySelectorAll('.btn[data-action="edit"]').forEach(btn => {
    btn.addEventListener("click", () => {
      const schoolId = btn.getAttribute("data-school-id");
      showComplianceModal(schools.find(s => s.id === schoolId), "edit", container);
    });
  });
}
...
function showComplianceModal(school, mode = "view", container) {
  const modalRoot = document.getElementById("compliance-modal-root") || document.body;
  const isEdit = mode === "edit";

  const lastAudit = school.lastAuditDate ? new Date(school.lastAuditDate).toISOString().slice(0,10) : "";
  const renewal = school.renewalDate ? new Date(school.renewalDate).toISOString().slice(0,10) : "";
  ...
  if (isEdit) {
    modal.querySelector("#compliance-form").onsubmit = async e => {
      ...
      const updated = {
        ...
        instructorFiles: fd.get("instructorFiles") ? fd.get("instructorFiles").split(',').map(x => x.trim()) : [],
