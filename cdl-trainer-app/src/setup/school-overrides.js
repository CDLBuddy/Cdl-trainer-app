...
(function initSchoolOverrides(win) {
  ...
  const normKey = (id) => String(id || '').trim().toLowerCase();
  ...
  const websites   = { ...asStringMap(DEFAULT_WEBSITES), ...asStringMap(ENV_WEBSITES),   ...existingWeb };
  const scheduling = {                              /* no defaults */ ...asStringMap(ENV_SCHEDULING), ...existingSch };
  ...
  if (seedId && !localStorage.getItem('schoolId')) {
    try { localStorage.setItem('schoolId', seedId); } catch {}
