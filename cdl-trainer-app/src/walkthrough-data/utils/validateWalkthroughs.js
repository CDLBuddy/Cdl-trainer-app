...
const SECTION_KEYS = new Set(['section', 'critical', 'passFail', 'hidden', 'steps'])
const STEP_KEYS = new Set([
  'label', 'stepLabel', 'script', 'mustSay', 'required', 'passFail', 'skip',
  'hidden', 'tags',
])

/** Quick stats (optional to read by callers; not required) */
function summarizeWalkthroughSections(data) {
  let sections = 0, steps = 0, required = 0, passFail = 0
...
export function validateWalkthrough(name, data, options) {
  ...
  data.forEach((section, sIdx) => {
    ...
    if (opts.strictKeys) {
      for (const k of Object.keys(section)) {
        if (!SECTION_KEYS.has(k)) {
          push(problems, path, `unknown key "${k}" on section (strictKeys).`, prefix)
        }
      }
    }
    ...
    if (!isStr(sectionKey) || sectionKey.length === 0) {
      push(problems, path, 'section.section must be a non-empty string.', prefix)
    } else {
      if (!opts.allowDuplicateSections) {
        if (sectionNames.has(sectionKey)) {
          push(problems, path, `section "${sectionKey}" is duplicated within "${name}".`, prefix)
        } else {
          sectionNames.add(sectionKey)
        }
      }
      if (isStr(sectionKey) && sectionKey.length > opts.maxStringLength) {
        push(problems, path, `section name too long (${sectionKey.length} > ${opts.maxStringLength}).`, prefix)
      }
    }
    ...
    section.steps.forEach((step, stIdx) => {
      ...
      if (opts.strictKeys) {
        for (const k of Object.keys(step)) {
          if (!STEP_KEYS.has(k)) {
            push(problems, spath, `unknown key "${k}" on step (strictKeys).`, prefix)
          }
        }
      }
      ...
      if (opts.requireStepScript) {
        if (!isStr(script) || script.length === 0) {
          push(problems, spath, 'script is required (non-empty string).', prefix)
        }
      ...
      if (isStr(script) && script.length > opts.maxStringLength) {
        push(problems, spath, `script too long (${script.length} > ${opts.maxStringLength}).`, prefix)
      }
      ...
      if (opts.requireLabel && (!isStr(label) || label.length === 0)) {
        push(problems, spath, 'label is required (string); stepLabel may be used as an alias.', prefix)
      } else {
        ...
        if (step.stepLabel != null && !isStr(step.stepLabel)) {
          push(problems, spath, 'stepLabel must be a string when provided.', prefix)
        }
      }
      if (isStr(label) && label.length > opts.maxStringLength) {
        push(problems, spath, `label too long (${label.length} > ${opts.maxStringLength}).`, prefix)
      }
      ...
      if (!opts.allowDuplicateStepLabels && isStr(label) && label) {
        ...
        if (seenLabels.has(key)) {
          push(problems, spath, `duplicate step label "${label}" within section "${sectionKey || '(unnamed)'}".`, prefix)
        } else {
          seenLabels.add(key)
        }
      }
      ...
      if ('tags' in step) {
        if (!Array.isArray(step.tags)) {
          push(problems, spath, 'tags must be an array of strings when present.', prefix)
        } else {
          for (let i = 0; i < step.tags.length; i++) {
            ...
            if (!isStr(raw)) {
              ...
              if (tag.length === 0) {
                ...
                push(problems, tpath, `tag too long (${tag.length} > ${opts.maxStringLength}).`, prefix)
              }
            }
          }
        }
      ...
      if (opts.warnPassFailNotRequired && step.passFail === true && step.required !== true) {
        push(problems, spath, 'is passFail but not marked required: consider required:true.', prefix)
      }
    })
  })
...
export function assertValidOrThrow(name, data, options) {
  ...
  if (!res.ok) {
    const err = new Error(`Invalid walkthrough "${name}":\n${formatProblems(res.problems)}`)
