...
function readonlyMap(map) {
  if (import.meta?.env?.DEV) {
    const thrower = () => {
      throw new Error('This Map is read-only: use exported helpers instead.')
    }
    map.set = thrower
    map.delete = thrower
    map.clear = thrower
...
export const listDefaultTokens = () =>
  Object.freeze(Object.keys(DEFAULT_WALKTHROUGHS))
  ...
  if (Array.isArray(w.sections)) {
    w.sections.forEach((s, i) => {
      ...
      if (!Array.isArray(s.steps))
        errors.push(`sections[${i}].steps must be an array`)
      if (Array.isArray(s.steps)) {
        s.steps.forEach((st, j) => {
          ...
          if (!norm(st.script))
            errors.push(`sections[${i}].steps[${j}].script is required`)
        })
      }
    })
  }
