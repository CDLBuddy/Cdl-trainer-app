...
/** Skip aggressive preloads on constrained networks */
function isConstrainedNetwork() {
  try {
    ...
    return /\b(slow-2g|2g|3g)\b/i.test(t)
  } catch {
    return false
  }
}

/** requestIdleCallback ponyfill */
...
// ----------------------- Role preload loaders --------------------------

async function _loadStudentPreload() {
  try {
    return await import('@student/preload.js')
  } catch {
    return null
  }
}
async function _loadInstructorPreload() {
  try {
    return await import('@instructor/preload.js')
  } catch {
    return null
  }
}
async function _loadAdminPreload() {
  try {
    return await import('@admin/preload.js')
  } catch {
    return null
  }
}
async function _loadSuperPreload() {
  try {
    return await import('@superadmin/preload.js')
  } catch {
    return null
  }
}

// Fallback above-the-fold pages when a role has no preload module.
  ...
  switch (role) {
    case 'student':
      await Promise.allSettled([
        ...
        _once(
          'student:walk',
          () => import('@student/walkthrough/Walkthrough.jsx')
        ),
    ...
    case 'admin':
      await Promise.allSettled([
        _once(
          'admin:dashboard',
          () => import('@admin/dashboard/AdminDashboard.jsx')
        ),
        _once(
          'admin:companies',
          () => import('@admin/companies/AdminCompanies.jsx')
        ),
        _once(
          'admin:companyShow',
          () => import('@admin/companies/CompanyDetail.jsx')
        ),
        _once(
          'admin:reports',
          () => import('@/admin/reports/AdminReports.jsx')
        ),
        _once('admin:billing', () => import('@admin/billing/Billing.jsx')),
        _once(
          'admin:settings',
          () => import('@admin/settings/AdminSettings.jsx')
        ),
        _once(
          'admin:walkthroughs',
...
/**
 * Preload routers + “above the fold” pages for a role.
...
export async function preloadRoutesForRole(roleInput) {
  ...
  await _once(routerKey, () =>
    role === 'student'
      ? import('@student/StudentRouter.jsx')
      : role === 'instructor'
        ? import('@instructor/InstructorRouter.jsx')
        : role === 'admin'
          ? import('@admin/AdminRouter.jsx')
          : import('@superadmin/SuperadminRouter.jsx')
  )

  // Skip heavy preloads on constrained networks
  if (isConstrainedNetwork()) return

  // Delegate to role preload module; fallback if missing
  let api = null
...
/** Preload *all* pages for a role (heavier). */
...
/** Targeted route name within a role's preloader, if available. */
export async function preloadRoleRoute(roleInput, routeName) {
  ...
  if (api?.preloadRoute) {
    await _once(`preload:route:${role}:${routeName}`, () =>
      api.preloadRoute(routeName)
    )
  }
}

/** Infer role from a path and preload relevant routes. */
...
/** Idle/session-aware warming (call once after login state known). */
...
/**
 * Intent prefetch helper for links/buttons.
 * Use on hover/focus/touch with a concrete `to` path.
 */
export function prefetchOnIntent(to) {
  const exec = () => preloadByPath(to)
  return {
    onMouseEnter: exec,
    onFocus: exec,
    onTouchStart: exec,
