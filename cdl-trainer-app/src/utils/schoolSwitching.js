...
export function applyBrandingForSchool(school) {
  try {
    ...
    window.dispatchEvent(new CustomEvent("branding:updated", { detail: { logoUrl, schoolName } }));
  } catch { /* no-op */ }
}
...
export function computeAllowedSchools(userRole, userSchoolIds, allSchools) {
  if (userRole === "superadmin" || userSchoolIds.includes("all")) return allSchools;
 ...
 */
export async function switchSchool(schoolId, { persistServer = true, schoolObj = null } = {}) {
  if (!schoolId) {
