...
export default function ProfileVerify() {
  ...
  const enrollmentPct = useMemo(() => getEnrollmentReadiness(profile || {}), [profile])
  ...
  const updateVerified = useCallback(async (sectionKey, checked) => {
  ...
  const updateNote = useCallback(async (sectionKey, text) => {
    if (!email || !profile) return
    try {
      setSaving(true)
      const notes = { ...(profile?.verified?.notes || {}), [sectionKey]: text }
      const next = { ...(profile.verified || {}), notes, by: actorEmail, at: new Date().toISOString() }
      await updateUserProfileFields(email, { verified: next }, actorEmail)
    } catch {
    ...
    return (
      <Shell title="Verify Student">
        <div style={{ padding: '1.25rem' }}>
          ...
          <button className="btn outline" onClick={() => navigate('/instructor/dashboard')}>⬅ Back</button>
        </div>
      </Shell>
    )
  }
  ...
  return (
    <Shell title="Verify Student">
      ...
      <div className="dashboard-card" style={{ padding: '1rem', display: 'grid', gap: 8, marginBottom: '1rem' }}>
        ...
        <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', color: '#444' }}>
          <span><b>CDL Class:</b> {cdl}</span>
          <span><b>Billing:</b> {billingMode}</span>
        </div>

        {/* Readiness bars */}
        <div style={{ display: 'grid', gap: 10, gridTemplateColumns: '1fr 1fr', alignItems: 'center', marginTop: 6 }}>
        ...
        <div style={{ marginTop: 2, fontSize: '.9rem', color: '#666' }}>
          {saving ? 'Saving…' :
            verified?.by ? <>Last verified by <b>{verified.by}</b> • <b>{formatDate(verified.at)}</b></> : '—'}
        </div>
      ...
      <div className="dashboard-card" style={{ padding: 0 }}>
        {SECTION_META.map(({ key, title }, idx) => {
          ...
          return (
            <section key={key} style={{ borderTop: idx ? '1px solid #eee' : 'none', padding: '1rem' }}>
              <SectionHeader
                title={title}
                status={status}
              ...
              <div style={{ marginTop: 10 }}>
                <label htmlFor={`note_${key}`} style={{ display: 'block', fontWeight: 500 }}>
  ...
  return (
    <div>
      ...
      <div role="progressbar" aria-valuemin={0} aria-valuemax={100} aria-valuenow={pct}
           style={{ position: 'relative', height: 10, background: bg, borderRadius: 999 }}>
        <div style={{ position: 'absolute', inset: 0, width: `${pct}%`, background: fg }} />
      </div>
    </div>
  )
}
