...
export default function InstructorProfile() {
  ...
  const assignedStudentsHtml =
    Array.isArray(assignedStudents) && assignedStudents.length
      ? assignedStudents.map((s, i) =>
          <div key={s.email || i}>#{i + 1}: {s.name || s.email}</div>
        )
  ...
  const sessionLogHtml =
    Array.isArray(sessionLog) && sessionLog.length
      ? sessionLog.map((s, i) =>
          <div key={i}>
            #{i + 1}: {s.date || "--"} &mdash; {s.type || "Session"} &mdash; {s.student || ""}
          </div>
        )
  ...
  return (
    <div className="screen-wrapper fade-in profile-page" style={{ maxWidth: 540, margin: "0 auto" }}>
      <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.2rem" }}>
        <span style={{ fontSize: "1.25em", fontWeight: 500, color: accent }}>{brand.schoolName || "CDL Trainer"}</span>
        {brand.logoUrl &&
          <img src={brand.logoUrl} alt="School Logo" className="dashboard-logo" style={{ maxWidth: 75, verticalAlign: "middle" }} />}
      </header>
      <h2 style={{ color: accent }}>
        👤 Instructor Profile <span className="role-badge instructor" style={{ background: accent, color: "#fff" }}>Instructor</span>
      </h2>
      {complianceAlert}
      ...
      >
        ...
        <label>
          Phone:
          <input type="tel" name="phone" defaultValue={phone} placeholder="(Optional)" />
        </label>
        <label>
          Experience:
        ...
        <details>
          ...
          <label>
            Days/Times Available:<br />
            <input type="text" name="availability" defaultValue={availability} placeholder="e.g. Mon-Fri, 8am-4pm" />
          </label>
        </details>
        <details>
          <summary>
            <strong>Instructor License Info</strong> {complianceMissing && <span style={{ color: "#e67c7c" }}>(Required)</span>}
          </summary>
          <label>
            CDL Class:
          ...
          <label>
            CDL License #:
            <input type="text" name="licenseNumber" defaultValue={licenseNumber} />
          </label>
          <label>
            License Expiration:
        ...
        <details>
          ...
          <textarea name="feedback" rows={3} defaultValue={feedback} placeholder="Feedback, notes, or suggestions..." />
        </details>
        <details>
        ...
        <details>
          ...
          <textarea name="adminNotes" rows={2} defaultValue={adminNotes} disabled placeholder="Visible to staff/admin only" />
        </details>
        <label>
          <input type="checkbox" name="active" checked={!!active} disabled />
          Active Instructor <span style={{ fontSize: "0.98em", color: "#888" }}>(Set by admin)</span>
        </label>
        <button className="btn primary wide" type="submit" style={{ background: accent, border: "none" }}>
          Save Profile
        </button>
        <button
