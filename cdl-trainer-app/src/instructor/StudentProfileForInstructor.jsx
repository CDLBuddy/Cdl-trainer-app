...
import ChecklistReviewModal from "../components/ChecklistReviewModal"; // Assume you move modal logic here
...
export default function InstructorStudentProfile() {
  ...
  return (
    <div className="screen-wrapper fade-in" style={{ maxWidth: 540, margin: "0 auto" }}>
      <h2>
        👤 Student Profile{" "}
        <span className="role-badge student">Student</span>
        <span className="role-badge instructor-view" title="You are viewing as an instructor">
          Instructor View
        </span>
      </h2>
      <div className="profile-meta" style={{ display: "flex", gap: "1.4rem", alignItems: "center", marginBottom: "1.4rem" }}>
        {profilePicUrl ? (
          <img
            src={profilePicUrl}
            alt="Profile"
        ...
        <div>
          ...
          <div style={{ color: "#aaa", fontSize: "0.96em" }}>{studentEmail}</div>
        </div>
      </div>
      <div className="progress-bar" style={{ marginBottom: "1.2rem" }}>
        <div className="progress" style={{ width: `${profileProgress || 0}%` }}></div>
      ...
      <div className="profile-details glass-card" style={{ padding: "1.2rem" }}>
        ...
        <div>
          <strong>Endorsements:</strong> {endorsements.length ? endorsements.join(", ") : "-"}
        </div>
        <div>
          <strong>Restrictions:</strong> {restrictions.length ? restrictions.join(", ") : "-"}
        </div>
        <div>
          <strong>CDL Permit:</strong> {cdlPermit === "yes" ? "✔️ Yes" : cdlPermit === "no" ? "❌ No" : "-"}
        </div>
        <div>
          <strong>Permit Photo:</strong>{" "}
          {permitPhotoUrl ? (
            <a href={permitPhotoUrl} target="_blank" rel="noopener noreferrer" style={{ color: "#b48aff", textDecoration: "underline" }}>
              View
            </a>
          ) : (
            "-"
        ...
        <div>
          ...
          {driverLicenseUrl ? (
            <a href={driverLicenseUrl} target="_blank" rel="noopener noreferrer" style={{ color: "#b48aff", textDecoration: "underline" }}>
              View
            </a>
          ) : (
            "-"
        ...
        <div>
          ...
          {medicalCardUrl ? (
            <a href={medicalCardUrl} target="_blank" rel="noopener noreferrer" style={{ color: "#b48aff", textDecoration: "underline" }}>
              View
            </a>
          ) : (
            "-"
        ...
        <div>
          ...
          {truckPlateUrl ? (
            <a href={truckPlateUrl} target="_blank" rel="noopener noreferrer" style={{ color: "#b48aff", textDecoration: "underline" }}>
              View
            </a>
          ) : (
            "-"
        ...
        <div>
          ...
          {trailerPlateUrl ? (
            <a href={trailerPlateUrl} target="_blank" rel="noopener noreferrer" style={{ color: "#b48aff", textDecoration: "underline" }}>
              View
            </a>
          ) : (
            "-"
        ...
        <div>
          <strong>Waiver Signed:</strong> {waiverSigned ? "✔️" : "❌"} ({waiverSignature || "-"})
        </div>
        <div>
          <strong>Notes:</strong> {studentNotes || "-"}
        </div>
      </div>
      <div className="glass-card" style={{ marginTop: "1.2rem", padding: "1.2rem" }}>
        <strong>Instructor Actions</strong>
        <div style={{ margin: "0.5em 0", display: "flex", flexWrap: "wrap", gap: "0.8em" }}>
          <button
          ...
          >
            ✔️ Approve Profile {actions.profile && <span style={{ color: "#3ecf8e", marginLeft: 7 }}>✅</span>}
          </button>
          <button
            className="btn outline"
          ...
          >
            ✔️ Approve Permit {actions.permit && <span style={{ color: "#3ecf8e", marginLeft: 7 }}>✅</span>}
          </button>
          <button
            className="btn outline"
          ...
          >
            ✔️ Approve Vehicle {actions.vehicle && <span style={{ color: "#3ecf8e", marginLeft: 7 }}>✅</span>}
          </button>
          <button
            className="btn outline"
          ...
          >
            👀 Review Walkthrough {actions.walkthrough && <span style={{ color: "#3ecf8e", marginLeft: 7 }}>✅</span>}
          </button>
        </div>
        <div style={{ margin: "0.7em 0 0.2em 0" }}>
          <textarea id="instructor-note" style={{ width: "100%", minHeight: 50 }} placeholder="Add a note for this student..."></textarea>
          <button className="btn" style={{ marginTop: 6 }} onClick={handleSaveNote}>
            💾 Save Note
          </button>
        </div>
        <div>
          <button className="btn" style={{ marginTop: 14 }} onClick={() => navigate("/instructor-dashboard")}>
            ⬅ Back to Dashboard
          </button>
        </div>
      </div>
      <div className="glass-card" style={{ marginTop: "1.1rem" }}>
        <button className="btn outline" onClick={() => setShowChecklistModal(true)}>
