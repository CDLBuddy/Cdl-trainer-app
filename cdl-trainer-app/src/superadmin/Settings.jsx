...
import { getUserRole } from "../utils/auth";      // <- same helper used elsewhere
import { showToast } from "../utils/ui-helpers";  // <- global toast, styled to your theme
...
async function getApiKeys() {
  const snap = await getDocs(query(collection(db, "apiKeys"), orderBy("createdAt", "desc")));
  return snap.docs.map((d) => ({ id: d.id, ...d.data() }));
}
async function createApiKey(label, schoolId = null) {
  ...
  await logSettingsChange("apiKey", { action: "created", label, schoolId: schoolId || null });
  return key;
}
...
function Section({ title, children, defaultOpen = true }) {
  return (
    <details open={defaultOpen} className="settings-section">
      <summary className="section-title"><strong>{title}</strong></summary>
...
function KeyRow({ item, schoolName, onRevoke, disabled }) {
  return (
    <li className="api-key-row">
      ...
      {item.label ? <span className="api-key-label">-- {item.label}</span> : null}
      {item.schoolId ? (
        <span className="api-key-school"> -- for {schoolName ?? item.schoolId}</span>
      ) : (
      ...
      <button
        className="btn outline"
        onClick={() => navigator.clipboard.writeText(item.key).then(
...
function SettingsForm({
  ...
  function handleSubmit(e) {
    ...
    if (!Number.isFinite(score) || score < 0 || score > 100) {
      showToast("Minimum Passing Score must be between 0 and 100.", 2500, "error");
      return;
    }
    onSubmit({
  ...
  return (
    <form id={id} className="settings-form" onSubmit={handleSubmit}>
      <label>
        Platform Logo URL:
        <input name="logoUrl" type="text" value={form.logoUrl} onChange={handleChange} />
      </label>
      <label>
        Primary Brand Color:
        <input name="brandColor" type="color" value={form.brandColor} onChange={handleChange} />
      </label>
      <label>
        Minimum Passing Score (%):
        <input name="minPassingScore" type="number" min="0" max="100" value={form.minPassingScore} onChange={handleChange} />
      </label>
      ...
      <label>
        ...
        <input name="certificateFooter" type="text" value={form.certificateFooter} onChange={handleChange} />
      </label>
...
export default function Settings() {
    ...
    async function load() {
      try {
        const [gs, scs, keys, logsSnap] = await Promise.all([
          ...
          getAllSchools(),
          getApiKeys(),
          getDocs(query(collection(db, "settingsLogs"), orderBy("changedAt", "desc"))),
        ]);
        if (!mounted) return;
  ...
  const selectedSchoolName = useMemo(
    () => (selectedSchoolId ? schools.find((s) => s.id === selectedSchoolId)?.name : ""),
  ...
  return (
    <div className="screen-wrapper fade-in" style={{ maxWidth: 820, margin: "0 auto", padding: 20 }}>
      <h2 className="dash-head">
        ⚙️ Super Admin: Settings & Overrides
        {saving ? <span style={{ marginLeft: 10, fontSize: ".8em", opacity: .8 }}>Saving…</span> : null}
      </h2>
      ...
      <Section title="API Key Management" defaultOpen={false}>
        <form id="api-key-form" onSubmit={handleCreateApiKey} style={{ marginBottom: "1em", display: "flex", gap: 10, flexWrap: "wrap" }}>
          <input name="label" type="text" placeholder="Label (e.g. 'TPR Integration')" required style={{ minWidth: 220 }} />
          ...
          <button className="btn" type="submit">Generate API Key</button>
        </form>
            ...
            {apiKeys.length === 0 ? (
              ...
              apiKeys.map((k) => (
                <KeyRow
                  key={k.id}
                  item={k}
                  schoolName={k.schoolId ? schools.find((s) => s.id === k.schoolId)?.name : undefined}
      ...
      <button className="btn outline" style={{ marginTop: "1.6rem" }} onClick={() => navigate("/superadmin")}>
        ⬅ Dashboard
      </button>
    </div>
  );
