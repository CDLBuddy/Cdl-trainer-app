...
export default function Compliance() {
  ...
  const view = useMemo(() => {
    ...
    if (tprStatus)
      rows = rows.filter(s => (s.tprStatus || 'Unknown') === tprStatus)
  ...
  return (
    <div
      className="screen-wrapper fade-in compliance-page"
      ...
      <div className="dashboard-card" style={{ overflow: 'hidden' }}>
        ...
        >
          <h3 style={{ margin: 0, marginRight: 'auto' }}>
            School Regulatory Compliance Overview
          </h3>
        ...
        <div className="dashboard-controls" style={{ marginTop: '0.9em' }}>
          ...
          <select
            value={tprStatus}
            onChange={e => setTprStatus(e.target.value)}
          >
        ...
        <div className="user-table-scroll" style={{ marginTop: '1rem' }}>
          <table
            className="user-table compliance-table"
            style={{ width: '100%' }}
          >
            <thead>
              <tr>
                ...
                view.map(s => {
                  ...
                  return (
                    <tr key={s.id}>
                      ...
                      <td>
                        {s.renewalDate ? badgeForRenewal(s.renewalDate) : '-'}
                      </td>
                      <td>
                        {s.lastAuditDate ? badgeForAudit(s.lastAuditDate) : '-'}
                      </td>
                      <td>
                        {s.curriculumFile ? (
                          <>
                            <a
                              href={s.curriculumFile}
                              target="_blank"
                              rel="noreferrer"
                            >
                              View
                      ...
                      <td>
                        <span
                          className={`compliance-health compliance-${score}`}
                        >
                          {score.toUpperCase()}
                        </span>
                      </td>
        ...
        <div style={{ margin: '1em 0 .3em', fontSize: '.98em', color: '#77a' }}>
          <b>Note:</b> All compliance changes are logged. Upload latest docs,
          flag issues, and keep audit/renewal dates up to date.
        </div>
