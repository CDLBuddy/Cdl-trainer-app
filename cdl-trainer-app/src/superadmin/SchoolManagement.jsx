...
async function getNextSchoolId() {
  ...
  const ids = snap.docs.map(d => d.id).filter(id => /^school\d+$/.test(id))
  const maxNum = Math.max(
    0,
    ...ids.map(id => parseInt(id.replace('school', ''), 10))
  )
...
function SchoolProfileModal({ school, onClose }) {
  return (
    <Modal onClose={onClose} maxWidth={560}>
      ...
      <div>
        <strong>TPR ID:</strong> {school.tprId || '-'}
      </div>
      <div>
        <strong>Status:</strong> <StatusBadge status={school.status} />
      </div>
      <div>
        <strong>Expiry:</strong> {school.tprExpiry || '-'}
      </div>
      <div>
      ...
      <div>
        <strong>Range:</strong> {school.rangeAddress || '-'}
      </div>
      <div>
...
function EditSchoolModal({ initial, onSave, onClose }) {
  ...
  return (
    <Modal onClose={onClose} maxWidth={520}>
      <h3>
        {isEditing ? 'Edit School / Provider' : 'Add New School / Provider'}
      </h3>
      <form
      ...
      >
        <input
          name="name"
          type="text"
          placeholder="School/Provider Name"
          value={form.name}
        ...
        <input
          name="tprExpiry"
          type="date"
          value={form.tprExpiry}
          onChange={updateField}
        />
        <input
          name="rangeAddress"
          type="text"
          placeholder="Range Address (optional)"
          value={form.rangeAddress}
          onChange={updateField}
        />
        <input
          name="contactName"
        ...
        <textarea
          name="notes"
          rows={2}
          placeholder="Internal notes, reminders, audit flags..."
          value={form.notes}
          onChange={updateField}
        />
...
export default function SchoolManagement() {
  ...
  async function handleSaveNew(form) {
    try {
      ...
      setSchools(prev =>
        [...prev, { id: newId, ...payload }].sort((a, b) =>
  ...
  async function handleDeleteOne(id) {
    if (!window.confirm('Delete this school/provider? This cannot be undone.'))
      return
    try {
  ...
  async function handleBulkDelete() {
    ...
    if (
      !window.confirm(`Delete ${selected.size} schools? This cannot be undone.`)
    )
      return
    try {
  ...
  return (
    ...
    >
      ...
      <div className="dashboard-card" style={{ marginBottom: '1.8em' }}>
        ...
        >
          <button className="btn" onClick={() => setEditSchool({})}>
            + Add School
          </button>
          <input
            type="search"
                ...
                <tr key={school.id}>
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                      ...
                      <button
                        className="btn danger"
                        onClick={() => handleDeleteOne(school.id)}
                      >
                        Delete
                      </button>
                    </div>
