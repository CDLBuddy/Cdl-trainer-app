...
export default function Permissions() {
  ...
  function setField(userId, field, value) {
    setUsers((prev) =>
      prev.map((u) => (u.id === userId ? { ...u, [field]: value, __dirty: true } : u))
    );
  }
  ...
  function toggleSchool(userId, schoolId) {
    setUsers((prev) =>
      prev.map((u) => {
        ...
        const next = has ? list.filter((x) => x !== schoolId) : [...list, schoolId];
  ...
  return (
    <div className="screen-wrapper fade-in" style={{ maxWidth: 1120, margin: "0 auto" }}>
      <h2 className="dash-head">
        🔑 Permissions & Roles <span className="role-badge superadmin">Super Admin</span>
      </h2>
      ...
      <div className="dashboard-card" style={{ marginBottom: "1.4em" }}>
        <div style={{ display: "flex", flexWrap: "wrap", gap: "1rem", alignItems: "center" }}>
          ...
          <button className="btn" onClick={() => navigate("/superadmin-dashboard")}>
            ⬅ Dashboard
          </button>
        </div>
      </div>
        ...
        <table className="user-table" style={{ minWidth: 1000 }}>
          ...
          <tbody>
            {!filteredUsers.length ? (
              <tr>
                <td colSpan={9} style={{ textAlign: "center", padding: "1.2em", color: "#789" }}>
                  No users found.
                </td>
              </tr>
            ) : (
              filteredUsers.map((u) => {
                ...
                return (
                  <tr
                    ...
                    style={{
                      background:
                        u.__dirty ? "rgba(187, 239, 255, 0.08)" : "transparent",
                  ...
                  >
                    ...
                    <td>{u.email || "--"}</td>
                    <td>
                      <span className={roleBadgeClass}>{u.role || "student"}</span>
                    </td>
                    <td>
                      <select
                      ...
                      >
                        {["superadmin", "admin", "instructor", "student", "custom"].map(
                          (r) => (
                    ...
                    <td>
                      <div
                        className="glass-card"
                        style={{ padding: 8, borderRadius: 10, maxHeight: 120, overflow: "auto" }}
                      >
                        {schools.map((s) => (
                          <label
                            key={s.id}
                      ...
                      <div style={{ display: "flex", flexWrap: "wrap", gap: ".5em" }}>
                        {PERMISSIONS_LIST.map((p) => (
                          <label key={p} style={{ fontWeight: 400 }}>
                            ...
                            />
                            <span style={{ marginLeft: 6 }}>{p.replace(/_/g, " ")}</span>
                          </label>
                        ))}
                      </div>
                    </td>
                    ...
                    <td>
                      <input
                        type="date"
                        value={u.expiryDate || ""}
                        onChange={(e) => setField(u.id, "expiryDate", e.target.value)}
                        style={{ width: 140 }}
                      />
                    </td>
                    <td>
                      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                        <button
                          ...
                          onClick={() => saveUser(u)}
                          title={!u.__dirty ? "No changes to save" : "Save changes"}
                        >
