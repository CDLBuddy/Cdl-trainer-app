...
const entries = {
  ...
  'walkthroughs-review-queue': () =>
    import('@superadmin/walkthroughs').then(m => ({
      default: m.SAReviewQueue,
    })),
  'walkthroughs-review-detail': () =>
    import('@superadmin/walkthroughs').then(m => ({
      default: m.SAReviewDetail,
    })),
}
...
export async function preloadSuperadminDashboard() {
  return _once('sa:dashboard', entries.dashboard)
}
export async function preloadSuperadminSchools() {
  return _once('sa:schools', entries.schools)
}
export async function preloadSuperadminUsers() {
  return _once('sa:users', entries.users)
}
export async function preloadSuperadminCompliance() {
  return _once('sa:compliance', entries.compliance)
}
export async function preloadSuperadminBilling() {
  return _once('sa:billing', entries.billing)
}
export async function preloadSuperadminSettings() {
  return _once('sa:settings', entries.settings)
}
export async function preloadSuperadminLogs() {
  return _once('sa:logs', entries.logs)
}
export async function preloadSuperadminPermissions() {
  return _once('sa:permissions', entries.permissions)
}
export async function preloadSuperadminWalkthroughs() {
  return _once('sa:walkthroughs', entries.walkthroughs)
}

// New back-compat style helpers for review pages (optional)
export async function preloadSAReviewQueue() {
  return _once(
    'sa:walkthroughs-review-queue',
    entries['walkthroughs-review-queue']
  )
}
export async function preloadSAReviewDetail() {
  return _once(
    'sa:walkthroughs-review-detail',
    entries['walkthroughs-review-detail']
  )
}

export async function preloadSuperadminCore() {
  return preloadAboveTheFold()
}
...
export function prefetchSuperadminByPath(path = '') {
  ...
  if (p.includes('/superadmin/walkthroughs/review/'))
    return entries['walkthroughs-review-detail']()
  if (p.includes('/superadmin/walkthroughs/review'))
    return entries['walkthroughs-review-queue']()
