...
function assertPathPrefix(item, role) {
  ...
  if (!ok) {
    console.warn(
      `[navConfig] "${item.label}" path "${item.to}" is not under "/${role}".`
    )
  }
}
function assertUniqueLabels(items, role) {
  ...
  for (const it of items) {
    ...
    if (seen.has(key)) {
      console.warn(
        `[navConfig] Duplicate label "${it.label}" in ${role} top nav.`
      )
    }
    seen.add(key)
  }
...
export const STUDENT_TOP_NAV = Object.freeze(
  /** @type {NavItem[]} */ ([
    ...
    {
      to: '/student/checklists',
      label: 'Checklists',
      icon: '📋',
      prefetchRole: 'student',
      preloadKey: 'checklists',
    },
    {
    ...
    {
      to: '/student/walkthrough',
      label: 'Walkthrough',
      icon: '🧭',
      prefetchRole: 'student',
      preloadKey: 'walkthrough',
    },
    {
...
export const INSTRUCTOR_TOP_NAV = Object.freeze(
  /** @type {NavItem[]} */ ([
    ...
    {
      to: '/instructor/checklist-review',
      label: 'Checklist Review',
      icon: '✅',
      prefetchRole: 'instructor',
      preloadKey: 'checklist-review',
    },
  ])
)
...
export const SUPERADMIN_TOP_NAV = Object.freeze(
  /** @type {NavItem[]} */ ([
    ...
    },
    {
      ...
      icon: '🛡️',
      prefetchRole: 'superadmin',
      preloadKey: 'compliance',
    },
    {
      to: '/superadmin/walkthroughs',
      label: 'Walkthroughs',
      icon: '🧭',
      prefetchRole: 'superadmin',
      preloadKey: 'walkthroughs',
...
export const NAV_REGISTRY = Object.freeze({
  ...
  instructor: Object.freeze({
    top: INSTRUCTOR_TOP_NAV,
    hidden: INSTRUCTOR_DEEP_LINKS,
  }),
  admin: Object.freeze({ top: ADMIN_TOP_NAV, hidden: ADMIN_DEEP_LINKS }),
  superadmin: Object.freeze({
    top: SUPERADMIN_TOP_NAV,
    hidden: SUPERADMIN_DEEP_LINKS,
  }),
})
