  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      const known = JSON.parse(
        localStorage.getItem('fcKnown_' + userEmail) || '[]'
      )
      ...
      setFlashcards(deck)
      setCurrent(
        !Number.isNaN(savedIdx) && savedIdx >= 0 && savedIdx < deck.length
          ? savedIdx
          : 0
      )
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        showToast(
          'Could not log your session, but your study progress is saved locally.'
        )
      }
    })()
