...
export default function StudentFlashcards() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setKnownCards(JSON.parse(localStorage.getItem("fcKnown_" + userEmail) || "[]"));
      ...
      setFlashcards(cards);
      setCurrent(!isNaN(savedIdx) && savedIdx >= 0 && savedIdx < cards.length ? savedIdx : 0);
  ...
  const handleShuffleToggle = (e) => {
    ...
    const newDeck = checked ? shuffleArray(defaultFlashcards) : [...defaultFlashcards];
      ...
      <div className="flashcards-complete container fade-in">
        ...
        <div className="flashcard-complete-actions">
          <button className="btn primary" onClick={() => {
            setKnownCards([]); localStorage.setItem("fcKnown_" + email, "[]");
            setCurrent(0); setStartedAt(Date.now()); setCompleted(false);
          }}>🔄 Restart</button>
          <button className="btn outline" onClick={() => {
            const missed = defaultFlashcards.filter((c, i) => !knownCards.includes(i));
            setFlashcards(missed); setKnownCards([]); setCurrent(0);
            setStartedAt(Date.now()); setCompleted(false);
  ...
  return (
    <div className="flashcards-page container fade-in">
      ...
      <div className="flashcard-controls">
        <button className="btn outline" onClick={handlePrev} disabled={current === 0}>← Prev</button>
        <button className="btn" onClick={handleFlip}>🔄 Flip</button>
        <button className="btn outline" onClick={handleNext} disabled={current === flashcards.length - 1}>Next →</button>
      </div>

      <div className="flashcard-actions">
        <button className="btn small" onClick={handleKnown}>✅ I know this</button>
        <button className="btn wide outline" onClick={handleEndSession}>✅ End Session</button>
        <button className="btn wide outline" onClick={() => navigate("/student-dashboard")}>⬅ Back to Dashboard</button>
      </div>
    </div>
  );
}