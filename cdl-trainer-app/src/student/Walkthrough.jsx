...
const Walkthrough = () => {
    ...
    return (
      <div className="screen-wrapper walkthrough-page fade-in">
        ...
        <div className="alert-box" role="alert">
          ...
          <br />
          Please go to your <strong>Profile</strong> and select one so we can
          load the correct walkthrough script.
        </div>
        <button className="btn" onClick={() => navigate('/student-profile')}>
          Go to Profile
        </button>
  ...
  if (!walkthroughData) {
    return (
      <div className="screen-wrapper walkthrough-page fade-in">
        <h2>🧭 CDL Walkthrough Practice</h2>
        <div className="alert-box" role="alert">
          ⚠ Sorry, we do not have a walkthrough script for your selected class:{' '}
          <b>{getWalkthroughLabel(cdlClass)}</b>.<br />
  ...
  return (
    <div className="screen-wrapper walkthrough-page fade-in" tabIndex={0}>
      ...
      <p>
        Study the walkthrough below to prepare for your in-person vehicle
        inspection test.{' '}
      ...
      <div className="walkthrough-script" aria-label="Walkthrough script">
        {walkthroughData.map((section, idx) => (
          <React.Fragment key={idx}>
            ...
            <div
              className={
                section.critical || section.passFail ? 'highlight-section' : ''
              }
            >
              {section.steps.map((step, i) => (
                <p key={i}>
                  ...
                  {step.passFail && (
                    <span
                      className="pass-fail"
                      style={{ color: '#ee3377', fontWeight: 'bold' }}
                    >
                      {' '}
                      (Pass/Fail)
                    </span>
      ...
      <div style={{ margin: '2rem 0 1.3rem 0' }}>
        ...
        ></progress>
        <span>{numCompleted}/4 drills completed</span>
      </div>
      <nav
        ...
        {parts.map((part, i) =>
          part.startsWith('___') && part.endsWith('___') ? (
            <input
              ...
              onChange={e =>
                handleInput(
                  parseInt(part.replace(/___/g, ''), 10),
                  e.target.value
                )
              }
              style={{ margin: '0 3px' }}
...
function OrderStepsDrill({ steps, onComplete, alreadyComplete }) {
  ...
  function checkOrder() {
    const correct = steps.every((step, i) => order[i] === step)
    setResult(
      correct
        ? '✅ Correct order!'
...
function TypePhraseDrill({ phrase, onComplete, alreadyComplete }) {
  ...
  return (
    <form onSubmit={check}>
      ...
      <div style={{ fontSize: '0.95em', marginTop: '0.6rem', opacity: 0.6 }}>
        Hint: {phrase}
      </div>
    </form>
  )
...
function VisualRecallDrill({
  ...
  function check() {
    const ok = value.trim().toLowerCase().includes(answer.toLowerCase())
