...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      if (role !== "admin" || !sid) {
        showToast("Access denied: Admin role and school required.", 3500, "error");
        if (window.handleLogout) window.handleLogout();
        navigate("/login", { replace: true });
        return;
      }
  ...
  const permitSoon = useMemo(() => users.filter((u) => expirySoon(u.permitExpiry)).length, [users]);
  const medSoon = useMemo(() => users.filter((u) => expirySoon(u.medCardExpiry)).length, [users]);
  const incomplete = useMemo(() => users.filter((u) => u.profileProgress < 80).length, [users]);
  ...
  return (
    <Shell title="Admin Dashboard">
      <div className={`${styles.wrapper} ${density === "compact" ? styles.compact : ""}`}>
        {/* Metrics */}
        <div className={styles.kpiRow}>
          <div className={styles.kpiCard}><b>👨‍🎓 Students:</b> <span>{studentCount}</span></div>
          <div className={styles.kpiCard}><b>👨‍🏫 Instructors:</b> <span>{instructorCount}</span></div>
          <div className={styles.kpiCard}><b>🛡 Admins:</b> <span>{adminCount}</span></div>
          <div className={`${styles.kpiCard} ${styles.warn}`}><b>🚨 Permit Soon:</b> <span>{permitSoon}</span></div>
          <div className={`${styles.kpiCard} ${styles.warn}`}><b>🚨 Med Card Soon:</b> <span>{medSoon}</span></div>
          <div className={styles.kpiCard}><b>📝 <span title="Profile < 80%">Incomplete Profiles</span>:</b> <span>{incomplete}</span></div>
        </div>
        ...
        <div className="u-toolbar" style={{ gap: 12 }}>
          <div className="u-field is-inline">
            ...
            <select className="u-select" value={roleFilter} onChange={(e) => { setRoleFilter(e.target.value); setPage(1); }}>
          ...
          <div className="u-field is-inline">
            <label className="u-label">Company</label>
            <select className="u-select" value={companyFilter} onChange={(e) => { setCompanyFilter(e.target.value); setPage(1); }}>
              <option value="">All</option>
              {companyList.map((c) => (
          ...
          <div className="u-field is-inline" style={{ flex: 1, minWidth: 220 }}>
            <label className="u-label">Search</label>
            <input
              className="u-input"
              type="search"
          ...
          <div className="u-field is-inline">
            <label className="u-label">Page size</label>
            <select className="u-select" value={pageSize} onChange={(e) => { setPageSize(Number(e.target.value)); setPage(1); }}>
              {[10, 25, 50, 100].map((n) => <option key={n} value={n}>{n}</option>)}
            </select>
          </div>

          <div className="u-field is-inline">
            <label className="u-label">Density</label>
          ...
          <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
            <button className="btn outline" onClick={() => exportUsersToCSV(filteredUsers)}>Export CSV</button>
            <button className="btn outline" onClick={() => exportUsersToPDF(filteredUsers)}>Export PDF</button>
            <button
        ...
        <div className="dashboard-card">
          <div className={styles.tableWrap}>
            <table className={styles.table} role="table">
              ...
              <tbody>
                {pageSlice.map((u) => (
                  <tr key={u.email} className={expirySoon(u.permitExpiry) ? styles.rowWarn : undefined}>
                    <td>{u.name}</td>
                    <td>{u.email}</td>
                    <td>
                      <select
                    ...
                    <td>
                      <input
                        ...
                        onChange={(e) => updateUser(u.email, { assignedCompany: e.target.value })}
                        placeholder="(Company)"
                      />
                    </td>
                    <td>
                      <select
                        ...
                        onChange={(e) => updateUser(u.email, { assignedInstructor: e.target.value })}
                      >
                    ...
                    <td className={expirySoon(u.permitExpiry) ? styles.expiry : undefined}>
                      {u.permitExpiry || ""}
                    </td>
                    <td className={expirySoon(u.medCardExpiry) ? styles.expiry : undefined}>
                      {u.medCardExpiry || ""}
                    </td>
          ...
          {/* Pagination */}
          <div className="u-flex u-flex-between u-wrap" style={{ marginTop: 12, gap: 10 }}>
            <div>
              Showing{" "}
              <b>
