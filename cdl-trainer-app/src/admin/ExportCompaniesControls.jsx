...
export default function ExportCompaniesControls({ companies, defaultType = "csv", className }) {
  ...
  const exportPDF = useCallback(async (list) => {
    ...
    (Array.isArray(list) ? list : []).forEach((c) => {
      const row = [c.name || "", c.contact || "", c.address || "", c.active ? "Yes" : "No"];
      row.forEach((cell, i) =>
  ...
  return (
    <div className={className} style={{ minWidth: 280 }}>
      ...
      <button className="btn" style={{ marginLeft: 7 }} onClick={handleDownload}>
        Download
      </button>
    </div>
  );
