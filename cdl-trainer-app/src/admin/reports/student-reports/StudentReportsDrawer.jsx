...
async function loadStudentReportMods() {
  try {
    const mod = await import('./student-reports/index.js')
    return {
    ...
    const [tpl, pdf] = await Promise.allSettled([
      import('./student-reports/cert-template.js'),
      import('./student-reports/pdf-utils.js'),
    ])
...
const normalizeTraining = (t = {}) => ({
  classType: S(t.classType || t.class || t.program || 'A').replace(/^class\s*/i,'').toUpperCase() || 'A',
...
const normalizeProvider = (p = {}) => ({
  ...
  tprId: S(p.tprId || (typeof window !== 'undefined' && window.__TPR_ID__) || ''),
  tin: S(p.tin || p.taxId || ''),
})
...
function StudentReportDrawer({
  ...
  React.useEffect(() => {
    let alive = true
    loadStudentReportMods().then((m) => { if (alive) setMods(m) })
    return () => { alive = false }
  }, [])
  ...
  const onBackdrop = (e) => { if (e.target === overlayRef.current) close() }
  ...
  const handleCopyJson = async () => {
    ...
    try {
      ...
      ok ? toast?.success?.('Certificate JSON copied') : toast?.error?.('Copy failed')
  ...
  const handleDownloadCsv = () => {
    if (!cert) return
    try {
      const filenameBase = (cert?.trainee?.lastName || (cert?.trainee?.fullName || 'student').split(' ').slice(-1)[0] || 'student')
      ...
      const row =
        (mods.buildTprCsvRow && mods.buildTprCsvRow(cert)) ||
        // minimal fallback row
        {
          fullName: cert?.trainee?.fullName || '',
          dob: cert?.trainee?.dob || '',
          licenseNumber: cert?.trainee?.licenseNumber || cert?.trainee?.clpNumber || '',
          licenseState: cert?.trainee?.licenseState || cert?.trainee?.clpState || '',
          classType: cert?.training?.classType || '',
          endorsement: cert?.training?.endorsement || '',
          completionDate: cert?.training?.completionDate || '',
        }
      const headers = Object.keys(row)
      const csvBody = (mods.toCsv && mods.toCsv([row], headers)) ||
        ...
        (() => {
          ...
          return [headers.join(','), headers.map((h) => esc(row[h])).join(',')].join('\r\n')
        })()
      ...
      mods.downloadCsv ? mods.downloadCsv(csv, name) : legacyDownload(csv, name)
      toast?.success?.('CSV downloaded')
    } catch { toast?.error?.('CSV build failed') }
  }
  ...
  const handlePrint = () => {
    ...
    try {
      ...
      toast?.info?.('Opened a new tab — use the browser Print dialog to save as PDF.')
    } catch { toast?.error?.('Unable to open printable view') }
  }
  ...
  return (
    <div
      ...
      style={reduceMotion ? { backdropFilter: 'none', WebkitBackdropFilter: 'none' } : undefined}
    >
      <aside
        ref={panelRef}
        role="document"
        ...
        style={reduceMotion ? { transition: 'none', transform: 'none', opacity: 1 } : undefined}
      >
        {/* Header */}
        <header className={styles.header}>
          <div>
            <div id={titleId} className={styles.title}>Student Completion</div>
            {trainee?.fullName && <div className={styles.subtitle}>{trainee.fullName}</div>}
          </div>
        ...
        <section className={styles.section}>
          ...
          <Field label="CLP/CDL #" value={trainee?.clpNumber || trainee?.licenseNumber} />
          <Field label="Issuing State" value={trainee?.clpState || trainee?.licenseState} />
        </section>
        ...
        <section className={styles.section}>
          <Field
            ...
            strong
          />
          <Field label="Class" value={trainingBlock?.classType ? `Class ${trainingBlock.classType}` : '—'} />
          <Field label="Endorsement" value={trainingBlock?.endorsement || '—'} />
          <Field label="Completion Date" value={trainingBlock?.completionDate || '—'} />
          <Field label="Theory Completed" value={trainingBlock?.theory?.completed ? 'Yes' : 'No'} />
          <Field label="BTW Completed" value={trainingBlock?.btw?.completed ? 'Yes' : 'No'} />
        </section>
      </aside>
    </div>
  )
...
function Field({ label, value, wide = false, strong = false }) {
  return (
    <div className={`${styles.field} ${wide ? styles.fieldWide : ''}`}>
      ...
      <div className={`${styles.fieldValue} ${strong ? styles.strong : ''}`}>{value || '—'}</div>
    </div>
  )
}
Field.propTypes = {
...
function validateCert(c) {
  ...
  if (!c?.trainee?.clpNumber && !c?.trainee?.licenseNumber) problems.push('CLP/CDL number is missing')
  if (!c?.trainee?.clpState && !c?.trainee?.licenseState) problems.push('Issuing state is missing')
  if (!c?.training?.classType) problems.push('Training class type (A/B/C) is missing')
