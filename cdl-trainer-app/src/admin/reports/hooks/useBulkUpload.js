...
export function mapCsvRowToCompletion(nr, provider = {}) {
  ...
  return {
    ...
    training: {
      ...
      endorsement: nr.endorsement && nr.endorsement !== 'NONE' ? nr.endorsement : '',
      completionDate: nr.completionDate || '',
      theory: { completed: nr.trainingType === 'theory' || nr.trainingType === 'both' },
      btw:    { completed: nr.trainingType === 'btw'    || nr.trainingType === 'both' },
    },
...
export function coerceIsoDate(s) {
  ...
  if (m) {
    const dd = Number(m[1]), mm = Number(m[2]), y = m[3]
    if (mm >= 1 && mm <= 12 && dd >= 1 && dd <= 31) return `${y}-${pad2(mm)}-${pad2(dd)}`
  }
...
function normalizeClassType(s) {
  const t = String(s || '').replace(/^class\s*/i, '').trim().toUpperCase()
...
export function normalizeRow(r = {}) {
  ...
  const dobRaw    = pick(r, ['dob', 'DOB', 'dateOfBirth', 'birthDate', 'dob(YYYY-MM-DD)'])
  ...
  const licenseState  = normalizeState(pick(r, ['licenseState', 'stateOfIssuance', 'state']))

  const clpNumber = pick(r, ['clpNumber', 'clp'])
  const clpState  = normalizeState(pick(r, ['clpState']))

  const trainingType = pick(r, ['trainingType', 'trainingType(theory|btw|both)']).toLowerCase()
  const classType    = normalizeClassType(pick(r, ['classType', 'classType(A|B|C)']) || 'A')
  const endorsement  = (pick(r, ['endorsement', 'endorsement(N|P|S|T|H|X|none)']) || 'NONE')
                        .toUpperCase()
  const completionRaw = pick(r, ['completionDate', 'completionDate(YYYY-MM-DD)'])
...
export function validateRow(nr) {
  ...
  else if (!/^\d{4}-\d{2}-\d{2}$/.test(nr.dob)) errs.push('dob(format YYYY-MM-DD)')
  ...
  if (!/^(theory|btw|both)$/.test(nr.trainingType || '')) errs.push('trainingType')
  if (!/^[ABC]$/.test(nr.classType || '')) errs.push('classType')

  if (!nr.completionDate) errs.push('completionDate')
  else if (!/^\d{4}-\d{2}-\d{2}$/.test(nr.completionDate)) errs.push('completionDate(format YYYY-MM-DD)')
...
export default function useBulkUpload(opts = {}) {
  ...
  const reset = useCallback(() => {
    setRows([]); setIssues([]); setFileName(''); lastFileRef.current = null
  }, [])

  /* --------------------------- CSV parsing --------------------------- */

  // Guard CSV type/size (use in dialog or here)
  const guardFile = useCallback((file) => {
    ...
    if (file.size > maxSizeMB * 1024 * 1024) return `CSV is too large (>${maxSizeMB} MB).`
    return ''
  }, [maxSizeMB])
    ...
    try {
      ...
      return await new Promise((resolve, reject) => {
        Papa.parse(fileObj, {
          ...
          step: undefined,
          complete: ({ data }) => resolve(Array.isArray(data) ? data.slice(0, maxRows) : []),
  ...
  const parseCsvFile = useCallback(async (file) => {
    ...
    const issues = []
    if (over) issues.push(`Trimmed to first ${maxRows} rows to keep things snappy.`)
    if (parsed?._truncated) issues.push(`Trimmed to first ${maxRows} rows to keep things snappy.`)
  ...
  const handleFile = useCallback(async (file) => {
    const err = guardFile(file)
    if (err) { setIssues([err]); setRows([]); setFileName(file?.name || ''); return { rows: [], issues: [err] } }
  ...
  const toCompletions = useCallback((provider = {}) => rows.map((r) => mapCsvRowToCompletion(r, provider)), [rows])

  return {
    // dialog controls
    open, openDialog, closeDialog, reset,

    // parsing state
    parsing, rows, issues, fileName, summary,

    // file handlers
    handleFile, parseCsvFile, guardFile,

    // helpers (exported above too)
    normalizeRow, validateRow, normalizeAndValidate,
    mapCsvRowToCompletion, toCompletions,
