...
export default function CertPreview({
  ...
  const licState =
    trainee.state || trainee.licenseState || trainee.clpState || '—'
  ...
  const program =
    cert.programType ||
    trn.programType ||
    (trn.btw?.completed && trn.theory?.completed
      ? 'Both'
      : trn.btw?.completed
        ? 'BTW'
        : trn.theory?.completed
          ? 'Theory'
          : '—')
  ...
  return (
    ...
    >
      ...
      <div className={styles.grid}>
        ...
        <Field
          label="Class"
          value={`Class ${String(classType).toUpperCase()}`}
        />
      ...
      <footer className={styles.footer}>
        <small className={styles.muted}>
          This is a read-only snapshot for quick review. Use the Student Details
          drawer for CSV/PDF.
        </small>
      </footer>
    </section>
