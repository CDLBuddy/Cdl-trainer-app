...
function UsersTable({
  ...
  return (
    <div className={styles.tableWrap} role="region" aria-label="Users table">
      <table className={styles.table} style={{ minWidth: 760 }} aria-busy={loading || undefined}>
        <thead>
          <tr>
            ...
            <th className={styles.th} scope="col">Company</th>
            <th className={styles.th} scope="col">Permit Expiry</th>
            <th className={styles.th} scope="col">Profile %</th>
            {onOpenStudent && <th className={styles.th} scope="col" style={{ width: 1 }} />}
          </tr>
        </thead>
        ...
        <tbody>
          {loading ? (
            // Skeleton rows
            Array.from({ length: Math.max(6, Math.min(10, data.length || 6)) }).map((_, i) => (
              <tr key={`skel-${i}`} aria-hidden>
                <td className={styles.td}><span className={styles.skel} style={{ width: 160 }} /></td>
                <td className={styles.td}><span className={styles.skel} style={{ width: 220 }} /></td>
                <td className={styles.td}><span className={styles.skel} style={{ width: 80 }} /></td>
                <td className={styles.td}><span className={styles.skel} style={{ width: 140 }} /></td>
                <td className={styles.td}><span className={styles.skel} style={{ width: 110 }} /></td>
                <td className={styles.td}><span className={styles.skel} style={{ width: 60 }} /></td>
                {onOpenStudent && <td className={styles.td} />}
              </tr>
            ))
          ) : data.length ? (
            data.map((u, i) => {
              const key = u?.id || u?.uid || u?.email || `${safeText(u?.name) || 'row'}-${i}`
              return <UserRow key={key} u={u} onOpenStudent={onOpenStudent} dateFmt={dateFmt} />
            })
          ) : (
            <tr>
              <td className={styles.td} colSpan={onOpenStudent ? 7 : 6}>
                <div style={{ padding: '1.25rem 0', textAlign: 'center', color: '#789' }}>
                  {emptyMessage}
                </div>
              </td>
            </tr>
...
const UserRow = React.memo(function UserRow({ u, onOpenStudent, dateFmt }) {
  const name = safeText(u?.name) || safeText(u?.fullName) || safeText(u?.displayName) || '—'
  ...
  return (
    <tr>
      <th className={styles.td} scope="row" style={{ fontWeight: 600 }}>{name}</th>
      ...
      <td className={styles.td}>
        <span className={styles.pill} style={roleStyle} title={`Role: ${roleLabel}`}>
          {roleLabel}
        </span>
      </td>
...
}, (prev, next) => prev.u === next.u && prev.onOpenStudent === next.onOpenStudent && prev.dateFmt === next.dateFmt)
...
function rolePill(role) {
  ...
  switch (role) {
    case 'student':
      return { label: 'Student', style: { background: 'rgba(78,145,173,.18)', color: 'var(--brand-light, #4e91ad)' } }
    case 'instructor':
      return { label: 'Instructor', style: { background: 'rgba(100,181,246,.18)', color: '#60a5fa' } }
    case 'admin':
      return { label: 'Admin', style: { background: 'rgba(72,187,120,.18)', color: 'var(--success,#48bb78)' } }
    case 'superadmin':
      return { label: 'Super Admin', style: { background: 'rgba(229,62,62,.18)', color: 'var(--error,#e53e3e)' } }
    default:
      return base
  }
}