...
export default function useEmployerBilling({ schoolId, showToast } = {}) {
  ...
  const [rows, setRows] = useState(
    /** @type {EmployerInvoice[]} */ (USE_BILLING_MOCKS ? mockEmployerInvoices() : [])
  )
  const [search, setSearch] = useState('')
  /** @type {[('all'|InvoiceStatus), React.Dispatch<React.SetStateAction<'all'|InvoiceStatus>>]} */
  const [status, setStatus] = useState(/** @type {'all'|InvoiceStatus} */('all')) // all|unpaid|partial|paid|void
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err : new Error('Failed to load employer invoices'))
    } finally {
  ...
  const filtered = useMemo(() => {
    ...
    if (status !== 'all') {
      out = out.filter(r => r.status === status || (status === 'unpaid' && r.status === 'partial'))
    }
