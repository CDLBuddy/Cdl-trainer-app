...
const WalkthroughManager  = lazy(() => import('@admin/walkthroughs/Manager/WalkthroughManager.jsx'))

// NEW: Full-screen schedule (admin)
const AdminSchedule = lazy(() => import('@admin/schedule/AdminSchedule.jsx'))
...
class AdminSectionErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, info) {
    if (__DEV__) console.error('[AdminRouter] render error:', error, info)
  }
...
export default function AdminRouter() {
  useEffect(() => { ensureModalRoot('modal-root') }, [])

  useEffect(() => {
    if (typeof window === 'undefined') return
    const prefersReduced = !!window.matchMedia?.('(prefers-reduced-motion: reduce)')?.matches
    const warmCore = () => { if (!prefersReduced) _preloadCore().catch(() => {}) }
    ...
    if ('requestIdleCallback' in window) {
      ...
      return () => { window.cancelIdleCallback?.(id); disposeReportsIdle?.() }
    }
    const t = setTimeout(warmCore, 300)
    return () => { clearTimeout(t); disposeReportsIdle?.() }
  }, [])
