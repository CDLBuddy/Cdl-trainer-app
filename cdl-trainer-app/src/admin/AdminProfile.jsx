...
const AdminProfile = () => {
  ...
  useEffect(() => {
    ;(async () => {
      ...
      // Completion %
      const completed = PROFILE_FIELDS.filter(f => !!data[f]).length
      setProfilePercent(Math.floor((completed / PROFILE_FIELDS.length) * 100))
      setLoading(false)
    })()
  ...
  const handleUpload = async (file, path, updateField) => {
    try {
      const fileRef = storageRef(
        storage,
        `${path}/${currentUserEmail}-${Date.now()}`
      )
      ...
      showToast(
        `Failed to upload ${updateField}: ${err.message}`,
        4200,
        'error'
      )
    }
  ...
  const handleDownloadPDF = async () => {
    ...
    docPDF.text(`${brand.schoolName || 'CDL Trainer'} - Admin Profile`, 10, 20)
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ marginBottom: '1.3em', padding: '1em' }}
      >
        <b>
        ...
        <small>
          ...
          {lastUpdated
            ? new Date(
                lastUpdated.seconds ? lastUpdated.seconds * 1000 : lastUpdated
              ).toLocaleString()
            : '(unknown)'}
        </small>
      </div>
      ...
      >
        ...
        <div className="logo-upload-section">
          ...
          <img
            ...
            src={
              brand.logoUrl || userData.companyLogoUrl || '/default-logo.svg'
            }
            alt="School Logo"
          ...
          <button
            ...
            onClick={handleSchoolLogoUpload}
          >
            Upload Logo
          </button>
        </div>
        ...
        <details>
          ...
          <label>
            ...
            <input
              ...
              defaultValue={userData.emergencyContactRelation || ''}
            />
          </label>
        </details>
        <details>
          ...
          <label>
            ...
            {userData.complianceDocsUrl && (
              <a
                href={userData.complianceDocsUrl}
                target="_blank"
        ...
        <label>
          <input
            type="checkbox"
            name="adminWaiverSigned"
            defaultChecked={!!userData.adminWaiverSigned}
          />
