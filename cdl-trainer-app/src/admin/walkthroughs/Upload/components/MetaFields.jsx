...
export default function MetaFields({ label, classCode, version, onLabel, onClassCode, onVersion }) {
  return (
    <div className={cls.metaGrid}>
      <label className={cls.metaField}>
        <span className={cls.metaLabel}>Label</span>
        <input className={cls.input} value={label} onChange={(e) => onLabel(e.target.value)}
      ...
      <label className={cls.metaField}>
        ...
        <input className={cls.input} type="number" min={1}
          value={version} onChange={(e) => onVersion(Number(e.target.value || 1))} />
      </label>
    </div>
  )
}