...
export function normalizeClassCode(v) {
  ...
  if (s.includes('PASSENGER') || s.includes('BUS') || s === 'P' || s === 'CLASS P' || s === 'PASSENGER-BUS') return 'PASSENGER-BUS'
...
export function rowsToSections(rows) {
  ...
  for (const r of rows) {
    const secName = String(r[SEC.find(k => k in r) ?? 'section'] ?? '').trim() || 'Untitled'
    ...
    groups.get(secName).push({
      ...
      mustSay, required, passFail, skip,
    })
  }

  return Array.from(groups.entries()).map(([section, steps]) => ({ section, steps }))
}
