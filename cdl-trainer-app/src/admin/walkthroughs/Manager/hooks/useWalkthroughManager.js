  ...
  return [
    ...
    },
    {
      ...
      script: [
        {
          section: 'Cab Safety',
          steps: [{ script: 'Seat belt…', required: true }],
        },
      ],
    },
  ]
...
export function useWalkthroughManager() {
  ...
  const toPreview = useCallback(id => {
    setActiveId(id)
    setView('preview')
  }, [])
  ...
  const handleImport = useCallback(
    dataset => {
      ...
      const row = {
        ...
        label:
          dataset?.label ||
          inferLabelFromToken(token) ||
          'Imported Walkthrough',
        classCode,
        token,
        ...
        prev.map(r =>
          r.id === id
            ? {
                ...
                ...(Array.isArray(patch?.script)
                  ? { script: cloneDeep(patch.script) }
                  : null),
                updatedAt: nowIso(),
              }
            : r
  ...
  const handleSubmit = useCallback(id => {
    setRows(prev =>
      prev.map(r =>
        r.id === id ? { ...r, status: 'in-review', updatedAt: nowIso() } : r
      )
    )
