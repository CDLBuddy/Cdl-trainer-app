...
export function validateScript(script) {
  ...
  if (!Array.isArray(script))
    return { ok: false, problems: ['Script must be an array of sections.'] }
  ...
  script.forEach((sec, si) => {
    ...
    if (!Array.isArray(sec.steps)) {
      problems.push(
        `Section "${sec.section || `#${si + 1}`}" steps must be an array.`
      )
      return
    }
    ...
    sec.steps.forEach((st, ti) => {
      if (!st || typeof st !== 'object') {
        problems.push(
          `Section "${sec.section || `#${si + 1}`}" step #${ti + 1} must be an object.`
        )
        return
      }
