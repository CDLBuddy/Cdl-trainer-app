  ...
  return (
    ...
    >
      ...
      <div title={item.id} className={cls.cellTrunc}>
        ...
        <div className={cls.subtle} aria-label="token">{item.token || item.id}</div>
      </div>

      {/* Class */}
      <div title={classCode} className={cls.cell}>{classLabel}</div>
      ...
      {/* Updated */}
      <div className={`${cls.cell} ${cls.subtle}`}>{fmtDate(item.updatedAt)}</div>

      {/* Actions */}
      <div className={cls.actions}>
        <button type="button" data-action="preview" onClick={() => onPreview?.(item.id)} title="Preview walkthrough">Preview</button>
        <button type="button" data-action="edit" onClick={() => onEdit?.(item.id)} title="Edit walkthrough">Edit</button>
        {st !== 'published' && (
          <button type="button" onClick={() => onSubmit?.(item.id)} title="Submit for review">Submit</button>
        )}
        <button type="button" onClick={() => onDuplicate?.(item.id)} title="Duplicate">Duplicate</button>
        <button type="button" onClick={() => onExport?.(item.id)} title="Export data">Export</button>
        {!item.isDefault && (
          <button
            type="button"
            onClick={() => {
              if (confirm('Delete this walkthrough? This cannot be undone.')) onDelete?.(item.id)
            }}
