...
export default function useCompanyDetailPage(companyId, showToast) {
  ...
  // enrich → filter → sort
  const enriched = useMemo(() => roster.map(r => ({
    ...r,
  ...
  const filtered = useMemo(() => {
    ...
    if (billingFilter !== 'all') rows = rows.filter(r => r._billingMode === billingFilter)
    if (onlyUnassigned) rows = rows.filter(r => !r._instructor || r._instructor === '—')
    return rows
