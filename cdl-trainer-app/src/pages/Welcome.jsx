...
import { getCurrentSchoolBranding, setCurrentSchool, getAllSchools } from "../utils/schoolBranding";
...
function SchoolSelectorModal({ open, onSelect, onClose }) {
  ...
  return (
    ...
    >
      <div className="modal-card school-select-modal" style={{ maxWidth: 410, margin: "10% auto" }}>
        ...
        <div style={{ marginBottom: "1.2rem" }}>
          ...
          >
            {schools.map(s => (
              <option value={s.id} key={s.id}>{s.schoolName}</option>
            ))}
          </select>
        </div>
        <button
...
function Welcome() {
  ...
  const [showSchoolSelector, setShowSchoolSelector] = useState(!localStorage.getItem("schoolId"));
  ...
  return (
    <div className="welcome-screen" tabIndex={0} aria-label="Welcome screen">
      ...
      <div className="switch-school-wrapper">
        <button className="btn outline" onClick={() => setShowSchoolSelector(true)}>
          Switch School
        </button>
      </div>
      ...
      <div className="bokeh-layer" aria-hidden="true">
        <div className="bokeh-dot parallax-float" style={{ top: "10%", left: "15%", animationDelay: "0s" }} />
        <div className="bokeh-dot parallax-float" style={{ top: "30%", left: "70%", animationDelay: "2s" }} />
        <div className="bokeh-dot parallax-float" style={{ top: "60%", left: "25%", animationDelay: "4s" }} />
        <div className="bokeh-dot parallax-float" style={{ top: "80%", left: "80%", animationDelay: "6s" }} />
      </div>
      ...
      <div className="welcome-content shimmer-glow fade-in" role="main">
        ...
        <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap", justifyContent: "center" }}>
          <button className="btn pulse" aria-label="Login" onClick={() => navigate("/login")}>
            <span className="icon">🚀</span> Login
          </button>
          <button className="btn outline" aria-label="Request a Demo" onClick={() => navigate("/demo")}>
            <span className="icon">📞</span> Request Demo
          </button>
          <button className="btn outline" aria-label="Contact Support" onClick={() => navigate("/contact")}>
            <span className="icon">✉️</span> Contact
          </button>
        </div>
        <div className="features" tabIndex={0} aria-label="Feature highlights" style={{ marginTop: 30 }}>
