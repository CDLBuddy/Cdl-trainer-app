...
export async function renderTestResults(container = document.getElementById("app")) {
  ...
  let isStaff = (userRole === "admin" || userRole === "instructor" || userRole === "superadmin" || userRole === "owner");
  ...
  try {
    ...
    for (const email of studentFilterEmails) {
      const snap = await getDocs(query(collection(db, "testResults"), where("studentId", "==", email)));
      snap.docs.forEach(d => {
        const data = d.data();
  ...
  if (isStaff) {
    document.getElementById("export-csv-btn")?.addEventListener("click", () => {
      ...
      results.forEach(r => {
        ...
        const pct = Math.round((r.correct / r.total) * 100);
        const date = r.timestamp.toLocaleDateString();
        rows.push([name, r.studentId, r.testName, `${pct}% (${r.correct}/${r.total})`, date]);
      });
      const csvContent = "data:text/csv;charset=utf-8," + rows.map(e => e.map(s => `"${String(s).replace(/"/g,'""')}"`).join(",")).join("\n");
...
async function showStudentDetailsModal(email) {
  ...
  try {
    const { collection, query, where, getDocs } = await import("https://www.gstatic.com/firebasejs/11.10.0/firebase-firestore.js");
    ...
    const checklist = [
      ...
      { label: "Vehicle Qualified",  value: data.vehicleQualified === "yes" ? "✅" : "❌" },
    ...
    modal.querySelector('.student-modal-content').innerHTML = `<h3>Error</h3><div>${err.message}</div>`;
  }
}